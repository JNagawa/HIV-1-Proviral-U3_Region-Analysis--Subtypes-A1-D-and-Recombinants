...
ALIGNMENT="${REPO_ROOT}/results/msa/illumina/mafft_aligned.fasta"  # input: MAFFT alignment from the msa stage
RESULTS_DIR="${REPO_ROOT}/results/motif_mapping/illumina"          # output: all motif-mapping results + summary go here
...
if [ ! -s "${ALIGNMENT}" ]; then                     # can't map motifs without the upstream alignment...
    echo "ERROR: no alignment at ${ALIGNMENT} (run scripts/msa/illumina/compare_msa_illumina.sh first)." >&2  # ...tell the user which stage to run first...
    exit 1                                           # ...and abort
fi
...
run_tool() {                                         # run one tool on one input, time it, count hits, and log a summary row
    ...
    case "${tool}" in                                # dispatch to the right wrapper and hit-counting rule per tool
        ...
        tfbstools)
            ...
            ;;
        gquad)
            measure_and_run "${timelog}" -- "${STAGE_DIR}/run_gquad.sh" "${input}" "${out}" > "${log}" 2>&1  # run gquad G4 prediction under timing
            exit_code=$?                              # capture its exit status
            ...
            ;;
        pqsfinder)
            measure_and_run "${timelog}" -- "${STAGE_DIR}/run_pqsfinder.sh" "${input}" "${out}" > "${log}" 2>&1  # run pqsfinder G4 prediction under timing
            exit_code=$?                              # capture its exit status
            n_hits=0                                  # default to zero predictions
            [ -s "${out}" ] && n_hits=$(grep -vc "^#" "${out}" 2>/dev/null || echo 0)  # count non-comment GFF3 lines as predictions
            ;;
    ...
    append_summary_row "motif_mapping_illumina" "${tool}" "${sample}" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${exit_code}" "${valid}" "${metric}"  # write this run's row to summary.tsv

    if [ "${sample}" = "positive_control" ] && [ "${valid}" -eq 0 ]; then  # a positive control that finds nothing is suspect
        case "${tool}" in                             # the severity differs between TFBS tools and G4 tools
            gquad|pqsfinder)
                echo "NOTE: ${tool} found zero G-quadruplex predictions on HXB2's own U3 -- unlike the TFBS tools, there's no literature-confirmed G4 site here to treat as a strict positive control, but worth a manual look if this seems implausible." >&2  # softer note: no known G4 to require here
                ;;
            *)
                echo "WARNING: ${tool} found zero motif hits on HXB2's own U3 (positive control failed) -- treat its subset numbers as suspect until this is investigated." >&2  # hard warning: JASPAR motifs are expected here
                ;;
        esac
    fi
}
