...
run_tool() {
    ...
    case "${tool}" in
        fimo)
            # run the shared FIMO wrapper under timing
        ...
        moods)
            # run the shared MOODS wrapper under timing
        ...
        tfbstools)
            # run the shared TFBSTools wrapper under timing
        ...
        gquad)
            # run the shared gquad G4-prediction wrapper under timing
            measure_and_run "${timelog}" -- "${RUN_DIR}/run_gquad.sh" "${input}" "${out}" > "${log}" 2>&1
            # capture exit status; default to zero predictions
            exit_code=$?; n_hits=0
            # count non-comment GFF3 lines as predictions
            [ -s "${out}" ] && n_hits=$(grep -vc "^#" "${out}" 2>/dev/null || echo 0) ;;
        pqsfinder)
            # run the shared pqsfinder G4-prediction wrapper under timing
    ...
    # assume invalid with zero hits until proven otherwise
    valid=0; metric="0 hits"
    # if the tool found something, mark valid and record the count
    ...
    if [ "${sample}" = "positive_control" ] && [ "${valid}" -eq 0 ]; then
        # severity differs between G4 tools and TFBS tools
        case "${tool}" in
            gquad|pqsfinder)
