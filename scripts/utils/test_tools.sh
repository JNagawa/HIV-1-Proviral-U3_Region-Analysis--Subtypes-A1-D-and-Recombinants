...
conda activate HIV_U3analysis                        # activate the env whose tools we want to verify

tools=("NanoFilt" "NanoStat" "porechop" "porechop_abi" "nanoQC" "prefetch" "fasterq-dump" "trimmomatic" "cutadapt" "kraken2" "ccs" "NanoPlot" "shiver_init.sh" "shiver_align.sh")  # the tools the pipeline expects to be installed
