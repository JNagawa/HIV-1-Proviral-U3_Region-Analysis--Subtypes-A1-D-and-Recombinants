...
# adapter-trimmed + quality/length-filtered reads
...
# need at least one adapter trimmer available
...
for SRR in "${SRR_ACCESSIONS[@]}"; do                # download each accession in turn
    ...
    # plain FASTQ present, or a valid gzipped one?
    ...
    # extract a single-end FASTQ from the .sra (Nanopore is single-end)
    ...
    # compress the FASTQ in place (-f overwrites any stale .gz)
...
for SRR in "${SRR_ACCESSIONS[@]}"; do                # trim + filter each sample
    ...
    # valid filtered output already present?
    ...
    # decompress -> quality/length-filter -> recompress (NanoFilt is stdin/stdout)
    gunzip -c "${TRIMMED}" | \
        NanoFilt \
            --quality "${MIN_QUALITY}" \
            --length "${MIN_LENGTH}" | \
    ...
    # only delete a real trimmed file, never the raw input
    if [ "${TRIMMED}" != "${FASTQ}" ]; then
        # drop the intermediate trimmed file to save space
        rm -f "${TRIMMED}"
    fi
...
if [ ! -s "${REF_FASTA}" ]; then
    ...
    # build the .fai index needed by bcftools (minimap2 indexes on the fly)
...
for SRR in "${SRR_ACCESSIONS[@]}"; do                # map each sample to HXB2 and call a consensus
    ...
    # long-read (map-ont) align, to BAM, coordinate-sort in one pipe
