...
# per-sample consensus sequences are available from stage 02 (preferring
...
RESULTS_DIR="${REPO_ROOT}/results/tool_comparison/03_msa"
...
for TOOL in mafft muscle clustalo; do
    ...
    append_summary_row "03_msa" "${TOOL}" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"
done
