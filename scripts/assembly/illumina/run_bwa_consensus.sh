...
bwa mem -t "${THREADS:-4}" "${REF_FASTA}" "${R1}" "${R2}" 2>"${OUTDIR}/${SRR}_bwa.log" | \
    samtools view -b - | samtools sort -o "${BAM_OUT}" || exit 1  # map reads to HXB2, convert to BAM, sort; bail if any stage fails
...
bcftools mpileup -Ou -f "${REF_FASTA}" "${BAM_OUT}" 2>"${OUTDIR}/${SRR}_bcftools.log" | \
    bcftools call -c -Oz -o "${VCF_OUT}" 2>>"${OUTDIR}/${SRR}_bcftools.log" || exit 1  # pile up bases and call variants against the reference
tabix -p vcf "${VCF_OUT}" || exit 1                  # index the VCF so bcftools consensus can read it
cat "${REF_FASTA}" | bcftools consensus "${VCF_OUT}" > "${CONSENSUS_OUT}" 2>>"${OUTDIR}/${SRR}_bcftools.log" || exit 1  # apply called variants onto the reference to make the consensus
