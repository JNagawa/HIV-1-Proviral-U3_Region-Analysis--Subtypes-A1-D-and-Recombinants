...
smalt='smalt'                                         # smalt mapper command (used only if mapper=smalt)
bwa='bwa'                                             # bwa mapper command (used only if mapper=bwa)
bowtie2='bowtie2'                                     # bowtie2 mapper command (used only if mapper=bowtie)
bowtie2_build='bowtie2-build'                         # command to build a bowtie2 index
samtools='samtools'                                   # samtools command for BAM/pileup handling
mafft='mafft'                                         # mafft command for multiple-sequence alignment
...
MafftArgsForPairwise='--maxiterate 1000 --localpair'  # mafft options for the accurate pairwise reference-to-HXB2 alignment
...
MinContigLength=300                                   # drop contigs (and weak blast hits) shorter than 300bp
...
MinGapSizeToSplitGontig=160                           # split an aligned contig at internal gaps this long or longer
...
MinContigFragmentLength=80                            # keep post-split contig fragments at least this long
...
# trimming off.
TrimToKnownGenome=true                                # trim contig ends that overhang the reference alignment
...
# merge results).
BlastTasks='megablast'                                # blast task(s) used when matching contigs to references

# Options to give blast when blasting the contigs; run your blastn command with
# -help to investigate possibilities.
ContigBlastArgs="-max_target_seqs 1 -word_size 17"    # extra blastn options for the contig-vs-reference search
...
ContigMinBlastOverlapToMerge='0.8'                    # fractional overlap at/above which two blast hits are merged
...
MafftTestingStrategy="MinAlnLength"                   # rule for choosing between the --add and --addfragments alignments

# Shall we trim adapaters and low quality bases from reads, using trimmomatic?
TrimReadsForAdaptersAndQual=true                      # enable Trimmomatic adapter/quality trimming of reads
...
IlluminaClipParams='2:10:7:1:true'                    # Trimmomatic ILLUMINACLIP adapter-clipping parameters
BaseQualityParams='MINLEN:50 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:20'  # Trimmomatic quality/length trimming parameters
# How many threads Trimmomatic should use (it sometimes multithreads unless told
# not to, which can be problematic on clusters).
NumThreadsTrimmomatic=1                                # pin Trimmomatic to 1 thread to stay well-behaved on clusters

# Shall we trim exact matches to PCR primers from the end of reads using fastaq?
TrimReadsForPrimers=true                              # enable trimming of exact PCR-primer matches from read ends
# Shall we also trim matches to the PCR primers that differ by a single base
# change? (This slows down the trimming step a lot.)
TrimPrimerWithOneSNP=false                            # do not also trim near-primer matches (1 SNP off) -- too slow

# Shall we clean (remove read pairs that look like contaminants)?
CleanReads=true                                       # remove read pairs flagged as likely contaminants
...
mapper="bwa"                                          # use bwa (already a pipeline dependency) as the read mapper
...
smaltIndexOptions="-k 15 -s 3"                        # smalt index options (kmer length, sampling step)
smaltMapOptions="-x -y 0.7 -j 0 -i 2000"              # smalt mapping options (independent mates, identity, insert range)
...
bowtieOptions="--local --maxins 2000 --no-discordant --no-unal --quiet"  # bowtie2 mapping options (used only if mapper=bowtie)
...
bwaOptions='-v 2'                                     # bwa mem options (quiet: warnings/errors only)
...
samtoolsReadFlags='-f 3 -F 4'                         # keep only properly paired, mapped reads (drop unmapped)
...
mpileupOptions='--no-BAQ --min-BQ 5 --max-depth 1000000'  # mpileup options: BAQ off, min base quality 5, high depth cap
...
MinCov1=15                                            # positions with fewer than 15 reads become '?' in the consensus
# The minimum coverage to use upper case for the base (to signal increased
# confidence)
MinCov2=30                                            # positions with >=30 reads are written upper-case (higher confidence)
...
MinBaseFrac=-1                                        # negative: always call the single most common base (no ambiguity codes)
...
# data...
deduplicate=false                                     # do not remove PCR/optical duplicate read pairs
...
remap=true                                            # remap reads to the first consensus for a refined final consensus
...
MapContaminantReads=false                             # do not separately map the removed contaminant reads
...
GiveHXB2coords=true                                   # add HXB2 coordinates to the base-frequencies output (HIV-specific)

# Shall we align the contigs to the consensus, for comparison?
AlignContigsToConsensus=false                         # do not produce a contigs-vs-consensus alignment
...
KeepPreMappingReads=false                             # let the pre-mapping reads keep the temp_ prefix (so they get cleaned up)
...
MaxContigGappiness=0.05                               # (deprecated auto mode) max gap fraction allowed inside a contig
MinContigHitFrac=0.9                                  # (deprecated auto mode) min fraction of a contig that must blast to HIV
...
MappedContaminantReadsSuffix='_ContaminantReads'      # suffix for the mapped-contaminant-reads output
BaseFreqsSuffix='_BaseFreqs.csv'                       # suffix for the base-frequencies table
BaseFreqsWGlobalSuffix='_BaseFreqs_ForGlobalAln.csv'  # suffix for base freqs with global-alignment coordinates
BaseFreqsWHXB2Suffix='_BaseFreqs_WithHXB2.csv'        # suffix for base freqs with HXB2 coordinates
InsertSizeCountsSuffix='_InsertSizeCounts.csv'        # suffix for the insert-size distribution table
CoordsDictSuffix='_coords.csv'                         # suffix for the coordinate-translation dictionary
...
GlobalAlnSuffix='_ForGlobalAln.fasta'                 # suffix for the consensus prepared for global alignment
...
CutContigFile='temp_HIVcontigs_cut.fasta'             # temp: contigs after cutting at blast-hit boundaries
TempContigAlignment1='temp_HIVcontigs_wRefs_MafftAdd.fasta'       # temp: contigs+refs aligned with mafft --add
TempContigAlignment2='temp_HIVcontigs_wRefs_MafftAddFrags.fasta'  # temp: contigs+refs aligned with mafft --addfragments
...
GappyRefWithExtraSeq='temp_GappyRefWithExtraSeq.fasta'  # temp: reference with gaps plus extra sequence
FlattenedContigs='temp_FlattenedContigs.fasta'        # temp: contigs flattened to a single reference frame
...
AllMappedContaminantReads='temp_ContaminantReads_IncUnmapped.sam'  # temp: contaminant reads incl. unmapped (SAM)
...
AlignmentForTesting='temp_test.fasta'                 # temp: alignment used when testing mafft strategies
ContigsWith1ref='temp_ContigsWith1ref.fasta'          # temp: contigs aligned with a single reference
