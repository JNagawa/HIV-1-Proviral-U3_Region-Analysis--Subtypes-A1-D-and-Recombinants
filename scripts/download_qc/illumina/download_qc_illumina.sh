...
REPO_ROOT="$(cd "${STAGE_DIR}/../../.." && pwd)"     # repo top-level (three dirs up), the base for every other path
...
RAW_DIR="${REPO_ROOT}/data/raw/illumina"                         # input: downloaded paired FASTQs live here
RESULTS_DIR="${REPO_ROOT}/results/download_qc/illumina"          # output: all QC results + summary go here
...
ADAPTER_FILE=$(compgen -G "${CONDA_PREFIX}/share/trimmomatic*/adapters/TruSeq3-PE-2.fa" | head -1)  # locate the TruSeq3 adapter FASTA in the conda env (path varies by version)
THREADS="${THREADS:-4}"                              # thread count; honour an externally-set THREADS, else default to 4
...
for SRR in $(subset_accessions illumina "${REPO_ROOT}/scripts/common/subset_samples.tsv"); do  # loop over just the Illumina accessions chosen for this comparison
    ...
    # trimmer runs next). ---
    FASTQC_PRE_DIR="${RESULTS_DIR}/fastqc_pre_out"   # shared output dir for the pre-trim baseline QC
    mkdir -p "${FASTQC_PRE_DIR}"                     # ensure it exists
    if [ ! -s "${FASTQC_PRE_DIR}/${SRR}_1_fastqc.html" ]; then  # skip if the report already exists (makes reruns idempotent)
        echo "=== FastQC (pre-trim) on ${SRR} ==="   # progress marker in the log
        fastqc "${R1}" "${R2}" --outdir "${FASTQC_PRE_DIR}" --threads "${THREADS}" --quiet \
            > "${RESULTS_DIR}/fastqc_pre_${SRR}.log" 2>&1  # QC both raw mates; redirect stdout+stderr to the log
    fi

    for TOOL in fastp trimmomatic; do                # run each trimmer on the same input for a head-to-head comparison
        ...
        if [ "${TOOL}" = "fastp" ]; then             # fastp branch (single command with equivalent thresholds)
            ...
            measure_and_run "${TIMELOG}" -- \
                fastp \
                    ...
                    --thread "${THREADS}" \
                > "${LOG}" 2>&1                       # trim + dedup with the shared Q15/len50 bar; emit JSON/HTML reports
        else                                         # trimmomatic branch (builds a step list first)
            TRIM_STEPS=""                            # accumulate Trimmomatic's ordered trimming steps here
            if [ -n "${ADAPTER_FILE}" ] && [ -f "${ADAPTER_FILE}" ]; then  # only add adapter clipping if the adapter FASTA was found
                TRIM_STEPS="ILLUMINACLIP:${ADAPTER_FILE}:2:30:10:2:True "  # TruSeq3 PE adapter-clip step (recommended params)
            fi
            ...
            measure_and_run "${TIMELOG}" -- \
                trimmomatic PE \
                    ...
                    ${TRIM_STEPS} \
                > "${LOG}" 2>&1                       # run Trimmomatic PE with the built step list; -summary feeds the metric below
        fi
        ...
        if [ -s "${R1_OUT}" ] && [ -s "${R2_OUT}" ] && gzip -t "${R1_OUT}" 2>/dev/null && gzip -t "${R2_OUT}" 2>/dev/null; then  # both mates non-empty and valid gzip
            R1_READS=$(zcat "${R1_OUT}" | wc -l)     # line count of forward mate (reads = lines/4)
            R2_READS=$(zcat "${R2_OUT}" | wc -l)     # line count of reverse mate
            if [ "${R1_READS}" = "${R2_READS}" ] && [ "${R1_READS}" -gt 0 ]; then  # mates must be equal and non-empty for a valid pair
                VALID=1                              # mark this tool's output valid
            fi
            if [ "${TOOL}" = "fastp" ] && [ -s "${OUTDIR}/${SRR}_fastp.json" ]; then  # fastp reports passed-reads in its JSON
                PASSED=$(grep -o '"passed_filter_reads"[^,}]*' "${OUTDIR}/${SRR}_fastp.json" | head -1 | grep -o '[0-9]*$')  # pull the first passed_filter_reads number
                [ -n "${PASSED}" ] && METRIC="${PASSED} reads passed"  # record it as the key metric if found
            elif [ "${TOOL}" = "trimmomatic" ] && [ -f "${OUTDIR}/${SRR}_trimmomatic_summary.txt" ]; then  # Trimmomatic reports surviving % in its summary
                METRIC=$(grep "Both Surviving Read Percent" "${OUTDIR}/${SRR}_trimmomatic_summary.txt" | awk '{print $NF"% surviving"}')  # extract the both-surviving percentage
            fi

            # --- Post-trim FastQC on this tool's own output ---
            FASTQC_POST_DIR="${RESULTS_DIR}/fastqc_post_${TOOL}_out"  # per-tool post-trim QC output dir
            mkdir -p "${FASTQC_POST_DIR}"            # ensure it exists
            fastqc "${R1_OUT}" "${R2_OUT}" --outdir "${FASTQC_POST_DIR}" --threads "${THREADS}" --quiet \
                > "${RESULTS_DIR}/fastqc_post_${TOOL}_${SRR}.log" 2>&1  # QC this tool's trimmed reads for a before/after comparison
        fi

        append_summary_row "download_qc_illumina" "${TOOL}" "${SRR}" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write this trimmer's row to summary.tsv
        ...
        if [ "${VALID}" -eq 1 ]; then                # only run downstream filtering when this trimmer produced usable reads
            DEDUP_R1="${R1_OUT}" DEDUP_R2="${R2_OUT}"  # default: feed the trimmer output straight in (fastp already deduped)
            if [ "${TOOL}" = "trimmomatic" ]; then   # Trimmomatic has no dedup, so add a fastp dedup-only pass first
                DEDUP_R1="${OUTDIR}/${SRR}_1.dedup.fastq.gz"  # deduped forward mate for the Trimmomatic arm
                DEDUP_R2="${OUTDIR}/${SRR}_2.dedup.fastq.gz"  # deduped reverse mate for the Trimmomatic arm
                bash "${REPO_ROOT}/scripts/utils/fastp_dedup.sh" \
                    "${R1_OUT}" "${R2_OUT}" "${DEDUP_R1}" "${DEDUP_R2}" \
                    "${OUTDIR}/${SRR}_dedup_fastp" \
                    > "${RESULTS_DIR}/dedup_${TOOL}_${SRR}.log" 2>&1  # run the fastp dedup-only wrapper on Trimmomatic's output
            fi

            KRAKEN_OUTDIR="${RESULTS_DIR}/kraken2_${TOOL}_out"          # per-trimmer Kraken2 output dir
            KRAKEN_TIMELOG="${RESULTS_DIR}/kraken2_${TOOL}_${SRR}.time" # timing file for this Kraken2 run
            measure_and_run "${KRAKEN_TIMELOG}" -- \
                ...
                > "${RESULTS_DIR}/kraken2_${TOOL}_${SRR}.log" 2>&1  # paired-end Kraken2 wrapper: drop host/bacterial reads
            KRAKEN_EXIT=$?                           # capture Kraken2's exit status
            parse_time_metrics "${KRAKEN_TIMELOG}"   # refresh WALLCLOCK_SEC / PEAK_RSS_MB from the Kraken2 timing file

            KRAKEN_VALID=0                           # default to invalid until checked
            KRAKEN_METRIC="n/a"                      # default key metric
            KRAKEN_R1_OUT="${KRAKEN_OUTDIR}/${SRR}_1.kraken_filtered.fastq.gz"  # host-removed forward mate
            KRAKEN_R2_OUT="${KRAKEN_OUTDIR}/${SRR}_2.kraken_filtered.fastq.gz"  # host-removed reverse mate
            if [ -s "${KRAKEN_R1_OUT}" ] && [ -s "${KRAKEN_R2_OUT}" ]; then  # both filtered mates present and non-empty
                KRAKEN_VALID=1                       # mark the Kraken2 output valid
                KRAKEN_METRIC=$(grep -o '^Kraken2 filtering.*retained[^.]*' "${RESULTS_DIR}/kraken2_${TOOL}_${SRR}.log" | tail -1)  # pull the retained-reads summary line from the log
                [ -z "${KRAKEN_METRIC}" ] && KRAKEN_METRIC="filtered, see ${KRAKEN_OUTDIR}/${SRR}.kreport"  # fall back to pointing at the kreport if that line is absent
            fi
            append_summary_row "download_qc_illumina" "kraken2_after_${TOOL}" "${SRR}" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${KRAKEN_EXIT}" "${KRAKEN_VALID}" "${KRAKEN_METRIC}"  # record the Kraken2 row
        fi
    done
done
