...
set -uo pipefail                                     # -u errors on unset vars, pipefail fails a pipe if any stage fails; no -e so one tool failing doesn't kill the whole run
...
RESULTS_DIR="${REPO_ROOT}/results/download_qc/oxfordnano"  # output: all QC results + summary go here
...
MIN_QUALITY=7                                        # NanoFilt mean-quality floor (Q7, permissive for noisier Nanopore reads)
MIN_LENGTH=200                                       # NanoFilt minimum read length (bp), drops short/degraded fragments
...
for SRR in $(subset_accessions nanopore "${REPO_ROOT}/scripts/common/subset_samples.tsv"); do  # loop over just the Nanopore accessions chosen for this comparison
    FASTQ="${RAW_DIR}/${SRR}.fastq.gz"               # expected raw single-end FASTQ for this accession
    ...
    TIMELOG="${RESULTS_DIR}/porechop_nanofilt_${SRR}.time"  # timing file for the porechop+nanofilt run
    LOG="${RESULTS_DIR}/porechop_nanofilt_${SRR}.log"  # captured stdout+stderr of the run
    mkdir -p "${OUTDIR}"                             # ensure the output dir exists
    TRIMMED="${OUTDIR}/${SRR}_trimmed.fastq.gz"      # intermediate adapter-trimmed reads (removed later)
    FILTERED="${OUTDIR}/${SRR}_filtered.fastq.gz"    # final quality/length-filtered reads

    echo "=== NanoPlot/NanoQC/NanoStat (pre) on ${SRR} ==="  # progress marker for the pre-filter QC block
    NanoPlot --fastq "${FASTQ}" --outdir "${RESULTS_DIR}/nanoplot_pre_out/${SRR}" --prefix "${SRR}_pre_" \
        ...
        > "${RESULTS_DIR}/nanoplot_pre_${SRR}.log" 2>&1  # length/quality plots of the raw reads (baseline)
    nanoQC -o "${RESULTS_DIR}/nanoqc_out/${SRR}" "${FASTQ}" > "${RESULTS_DIR}/nanoqc_${SRR}.log" 2>&1  # per-base quality across read positions on raw reads
    NanoStat --fastq "${FASTQ}" --outdir "${RESULTS_DIR}/nanostat_out" --name "${SRR}_pre_stats.txt" \
        --threads "${THREADS}" > "${RESULTS_DIR}/nanostat_pre_${SRR}.log" 2>&1  # text summary stats of the raw reads
    ...
    ' _ "${FASTQ}" "${TRIMMED}" "${FILTERED}" "${MIN_QUALITY}" "${MIN_LENGTH}" "${THREADS}" \
        > "${LOG}" 2>&1                               # adapter-trim (porechop_abi, or porechop, or raw) then NanoFilt quality/length-filter, all timed as one step
    EXIT_CODE=$?                                     # capture the pipeline's exit status before $? is overwritten
    parse_time_metrics "${TIMELOG}"                  # set WALLCLOCK_SEC / PEAK_RSS_MB globals from the .time file
    rm -f "${TRIMMED}"                               # drop the intermediate trimmed file; only the filtered output is kept

    VALID=0                                          # assume invalid until proven otherwise
    METRIC="n/a"                                     # default key metric
    if [ -s "${FILTERED}" ] && gzip -t "${FILTERED}" 2>/dev/null; then  # filtered output must be non-empty and a valid gzip
        ...
        if [ "${FILT_READS}" -gt 0 ]; then           # at least one read must survive for a valid result
            VALID=1                                  # mark the output valid
            RETAINED=$(awk -v f="${FILT_READS}" -v r="${RAW_READS}" 'BEGIN{printf "%.1f", 100*f/r}')  # percent of reads retained
            METRIC="${FILT_READS}/${RAW_READS} reads retained (${RETAINED}%)"  # human-readable key metric
        fi

        echo "=== NanoPlot/NanoStat (post) on ${SRR} ==="  # progress marker for the post-filter QC block
        NanoPlot --fastq "${FILTERED}" --outdir "${RESULTS_DIR}/nanoplot_post_out/${SRR}" --prefix "${SRR}_post_" \
            ...
            > "${RESULTS_DIR}/nanoplot_post_${SRR}.log" 2>&1  # length/quality plots of the filtered reads (for before/after)
        NanoStat --fastq "${FILTERED}" --outdir "${RESULTS_DIR}/nanostat_out" --name "${SRR}_post_stats.txt" \
            --threads "${THREADS}" > "${RESULTS_DIR}/nanostat_post_${SRR}.log" 2>&1  # text summary stats of the filtered reads
    fi

    append_summary_row "download_qc_oxfordnano" "porechop_abi+nanofilt" "${SRR}" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write this sample's row to summary.tsv
done
...
multiqc "${RESULTS_DIR}" --outdir "${RESULTS_DIR}/multiqc_out" --filename "nanopore_qc_report" \
    --title "PRJNA765218 - Nanopore QC Summary (subset)" --force > "${RESULTS_DIR}/multiqc.log" 2>&1  # aggregate all per-sample QC into one report; --force overwrites a prior run
