...
ALIGNMENT="${REPO_ROOT}/results/msa/illumina/mafft_aligned.fasta"        # input: whole-subset MAFFT alignment from the msa stage (feeds IQ-TREE2)
RESULTS_DIR="${REPO_ROOT}/results/subtyping/illumina"                    # output: all subtyping results + summary go here
...
for SRR in $(subset_accessions illumina "${REPO_ROOT}/scripts/common/subset_samples.tsv"); do  # loop over just the Illumina accessions chosen for this comparison
    ...
    [ -s "${QUERY}" ] || { echo "NOTE: no consensus for ${SRR}, skipping (run scripts/assembly/illumina/compare_assembly_illumina.sh first)." >&2; continue; }  # skip samples with no consensus yet
    ...
    append_summary_row "subtyping_illumina" "jphmm" "${SRR}" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write this sample's jpHMM row to summary.tsv
done
...
if [ -s "${ALIGNMENT}" ]; then                       # only build a tree if the msa stage produced an alignment
    ...
    append_summary_row "subtyping_illumina" "iqtree2" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write the IQ-TREE2 row to summary.tsv
else
    echo "NOTE: no alignment at ${ALIGNMENT}, run scripts/msa/illumina/compare_msa_illumina.sh first." >&2  # tell the user to build the alignment first
fi
