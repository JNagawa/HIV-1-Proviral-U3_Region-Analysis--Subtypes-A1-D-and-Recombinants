...
ASSEMBLY_DIR="${REPO_ROOT}/results/assembly/illumina/bwa_consensus_out"   # input: per-sample consensus FASTAs from the Illumina assembly stage
RESULTS_DIR="${REPO_ROOT}/results/biological_filtering/illumina"          # output: all classification results + summary go here
...
N_SEQS=$(grep -c "^>" "${INPUT_FASTA}" 2>/dev/null || echo 0)  # count FASTA records (>= headers) to check we have enough input
...
append_summary_row "biological_filtering_illumina" "poplars" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write Poplars' row to summary.tsv
...
append_summary_row "biological_filtering_illumina" "hivseqinr" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write HIVSeqinR's row to summary.tsv
...
measure_and_run "${TIMELOG}" -- "${STAGE_DIR}/run_hivintact.sh" "${INPUT_FASTA}" "${OUTDIR}" A1 > "${RESULTS_DIR}/hivintact_wrapper.log" 2>&1  # run HIVIntact wrapper with subtype A1 (this cohort is A1/D, not B)
EXIT_CODE=$?                                          # capture the wrapper's exit status
parse_time_metrics "${TIMELOG}"                       # refresh WALLCLOCK_SEC / PEAK_RSS_MB from the timing file
VALID=0; METRIC="n/a"                                 # default to invalid until checked
...
append_summary_row "biological_filtering_illumina" "hivintact" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write HIVIntact's row to summary.tsv
