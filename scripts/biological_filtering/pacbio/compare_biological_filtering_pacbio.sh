...
RESULTS_DIR="${REPO_ROOT}/results/biological_filtering/pacbio"             # output: all classification results + summary go here
...
if [ ! -d "${REPO_ROOT}/scripts/tools/Poplars" ]; then  # the tool clones are a prerequisite
    echo "NOTE: tools not set up yet -- run scripts/biological_filtering/illumina/setup_tools.sh first." >&2  # point the user at the shared setup script
...
measure_and_run "${TIMELOG}" -- "${RUN_DIR}/run_poplars.sh" "${INPUT_FASTA}" "${OUT}" > "${RESULTS_DIR}/poplars.log" 2>&1  # run the shared Poplars wrapper under timing, capturing output
...
append_summary_row "biological_filtering_pacbio" "poplars" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write Poplars' row to summary.tsv
...
measure_and_run "${TIMELOG}" -- "${RUN_DIR}/run_hivseqinr.sh" "${INPUT_FASTA}" "${OUTDIR}" > "${RESULTS_DIR}/hivseqinr_wrapper.log" 2>&1  # run the shared HIVSeqinR wrapper under timing, capturing output
...
append_summary_row "biological_filtering_pacbio" "hivseqinr" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write HIVSeqinR's row to summary.tsv
...
measure_and_run "${TIMELOG}" -- "${RUN_DIR}/run_hivintact.sh" "${INPUT_FASTA}" "${OUTDIR}" A1 > "${RESULTS_DIR}/hivintact_wrapper.log" 2>&1  # run the shared HIVIntact wrapper with subtype A1 (this cohort is A1/D, not B)
EXIT_CODE=$?                                          # capture the wrapper's exit status
parse_time_metrics "${TIMELOG}"                       # refresh WALLCLOCK_SEC / PEAK_RSS_MB from the timing file
VALID=0; METRIC="n/a"                                 # default to invalid until checked
if [ -s "${OUTDIR}/intact.fasta" ] || [ -s "${OUTDIR}/nonintact.fasta" ]; then  # valid if either the intact or non-intact output was produced
    ...
    N_INTACT=$(grep -c "^>" "${OUTDIR}/intact.fasta" 2>/dev/null); N_INTACT="${N_INTACT:-0}"        # count intact proviruses (0 if file missing)
    N_NONINTACT=$(grep -c "^>" "${OUTDIR}/nonintact.fasta" 2>/dev/null); N_NONINTACT="${N_NONINTACT:-0}"  # count non-intact proviruses (0 if file missing)
    METRIC="${N_INTACT} intact, ${N_NONINTACT} non-intact"  # record the intact/non-intact split as the key metric
fi
append_summary_row "biological_filtering_pacbio" "hivintact" "all_subset" "${WALLCLOCK_SEC}" "${PEAK_RSS_MB}" "${EXIT_CODE}" "${VALID}" "${METRIC}"  # write HIVIntact's row to summary.tsv
