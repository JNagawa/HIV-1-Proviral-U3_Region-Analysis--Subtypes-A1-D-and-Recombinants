...
source "${REPO_ROOT}/tool_comparison/common/lib_compare.sh"

RAW_DIR="${REPO_ROOT}/illumina_raw_data"
RESULTS_DIR="${STAGE_DIR}/results"
SUMMARY_TSV="${RESULTS_DIR}/summary.tsv"
mkdir -p "${RESULTS_DIR}"
[ -f "${RESULTS_DIR}/ease_of_use_notes.md" ] || cp "${REPO_ROOT}/tool_comparison/common/ease_of_use_template.md" "${RESULTS_DIR}/ease_of_use_notes.md"

ADAPTER_FILE=$(compgen -G "${CONDA_PREFIX}/share/trimmomatic*/adapters/TruSeq3-PE-2.fa" | head -1)
THREADS="${THREADS:-4}"
export THREADS

for SRR in $(subset_accessions illumina "${REPO_ROOT}/tool_comparison/subset_samples.tsv"); do
    ...
    for TOOL in fastp trimmomatic; do
        ...
        if [ -s "${R1_OUT}" ] && [ -s "${R2_OUT}" ] && gzip -t "${R1_OUT}" 2>/dev/null && gzip -t "${R2_OUT}" 2>/dev/null; then
            ...
            if [ "${TOOL}" = "fastp" ] && command -v python3 >/dev/null 2>&1; then
                METRIC=$(python3 -c "
import json
d = json.load(open('${OUTDIR}/${SRR}_fastp.json'))
print(f\"{d['filtering_result']['passed_filter_reads']} reads passed\")
" 2>/dev/null || echo "n/a")
